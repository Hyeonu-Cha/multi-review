...
- **CLAUDE.md / convention compliance (externals can't see it).** Open every `CLAUDE.md` that shares a path with a changed file — the changed file's own directory **and each parent up to the repo root** — and flag changes that break a rule, **quoting the exact rule violated** and linking the file. Only rules whose path scope actually covers the changed file count; a rule under `src/api/CLAUDE.md` does not govern `src/web/`. This is convention adherence, not generic style — skip anything the rule doesn't explicitly state.
