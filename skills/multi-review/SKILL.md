...
- **Missing security/guard gate vs siblings (do not skip this — isolated reviewers are blind to it).** For every new or changed endpoint/handler, open the *other* handlers in the same controller **and the sibling controllers in the same folder** (Grep the directory) and check it applies the same preamble before doing work: auth/session validation, a `FailedInitialChecks`-style precondition, input validation, ownership/tenant checks, the same order of guard calls (e.g. `FailedInitialChecks` *before* `TryConvert`). An auth/session gate the siblings enforce and this handler omits means an unauthenticated/unauthorized caller can reach the action — that's `high`/`critical`. The gate lives in **unchanged** files, so only your repo access can see it.
