...
- **Wiring/registration vs consumption.** When a changed line registers or configures a dependency (DI container, route table, hook, config binding), open the consumers: does anything ask for a type/name/signature the registration doesn't provide (factory registered, concrete type injected; scoped service captured by a singleton)? These compile fine and die at startup — at least `high`.
